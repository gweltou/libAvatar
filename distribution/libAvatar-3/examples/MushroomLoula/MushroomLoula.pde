...
PRenderer renderer = new PRenderer(this);
int counter = 0;
...
Avatar createMushroom(float x) {
  ComplexShape mushroomShape = new ComplexShape("mushroom");
  ComplexShape original = Avatar.fromFile(sketchFile("data/mushroom.json")).getShape();
  ...
  Avatar mushroom;
  mushroom = new Avatar();
  mushroom.setShape(mushroomShape);
